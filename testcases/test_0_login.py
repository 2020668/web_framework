...
class TestLogin:
    ...
    def test_login_2_success(self, init_driver):  # fixture的函数名称，作为参数传给测试用例。函数名称=fixture执行后返回值。
        ...
        LoginPage(init_driver).login(ld.success_data["user"], ld.success_data["pwd"])  # 测试对象+测试数据
    ...
    def test_my_fix1(self, my_mod):
        print("11111111111111111111111111111111")
        print("使用整个测试会话级的返回值：", my_mod)

    # 异常用例 -....
    # @ddt.data(*ld.wrong_datas )
    @pytest.mark.parametrize("data", ld.wrong_datas)   # 参数化实现ddt
    def test_login_0_failed_by_wrong_data(self, data, init_driver):
        # 步骤 - 登陆操作 - 登陆页面 - 密码为空 18684720553
        LoginPage(init_driver).login(data["user"], data["pwd"])
        # 断言 - 页面的提示内容为：请输入密码
        assert data["check"] == LoginPage(init_driver).get_error_msg_from_login_form()

    # @ddt.data(*ld.fail_data)
    @pytest.mark.parametrize("data", ld.fail_datas)  # 参数化实现ddt
    def test_login_1_failed_by_fail_data(self, data, init_driver):
        # 步骤 - 登陆操作 - 登陆页面 - 密码为空 18684720553
        LoginPage(init_driver).login(data["user"], data["pwd"])
        # 断言 - 页面的提示内容为：请输入密码
        assert data["check"] == LoginPage(init_driver).get_error_msg_from_page_center()
